...
const lastFrame = (c: FakeConn): Frame => {
...
const lastError = (c: FakeConn): ErrorMessage => {
...
async function open(): Promise<[RoomHub, FakeConn, string]> {
...
async function seat(hub: RoomHub, code: string, n: number): Promise<FakeConn[]> {
...
function at(conns: FakeConn[], i: number): FakeConn {
  const conn = conns[i];
